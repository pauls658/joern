...
class G {
    ...
    def ctxInsensitiveForwardTrav(starts) {
    ...
    def backTravUntilUnhandleable(start, sensitiveReachable) {
        ...
        .repeat(
            ...
            .union(__.in("REACHES"), __.in("INTERPROC"))\
    	).until(filter{it.sack().haveVisited(it.get().id()) ||
				!(it.get().id() in sensitiveReachable) ||
				it.get().id() in it.sack().nset}) 
    	.sack().toList()[0]
		return res;
    }

    def ctxInsensitiveBackwardTrav(start, sensitiveReachable) {
		BSackObject s = new BSackObject(); 
        def res = g\
        .withSack{s}\
        .V(start)\
	...
	def getArgsRetsByName(String funcName) {
		...
		//.in('ASSOC').has("type", "return").toList()
		return nodes
	}
	...
	def analysis() {
		...
		print "Computing reachable statements from sensitive sources... "
		FSackObject fsack = ctxInsensitiveForwardTrav(startNodes);
		HashSet<Long> sensitiveReachable = fsack.visited;
		println "Done."
		print "Computing statements that can reach echos... "
		BSackObject bsack = ctxInsensitiveBackwardTrav(fsack.echos, sensitiveReachable);
		println "Done."
		HashSet<Long> intersection = sensitiveReachable.intersect(bsack.visited);
		print "Computing statements that can reach unhandleable nodes... "
		HashSet<Long> n1 = simpleBackTrav(bsack.nset, intersection);
		println "Done."
		HashSet<Long> n2 = intersection.minus(n1);
		print "Computing crossing edges... "
	...
	def testReachability() {
		...
		.where(
			and(
				union(out('REACHES'), out('INTERPROC')).count().is(0),
				union(__.in('REACHES'), __.in('INTERPROC')).count().is(gt(0))
			)
		)
	}
