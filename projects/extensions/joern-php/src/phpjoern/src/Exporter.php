...
abstract class Exporter {
  ...
  /** A list of function ids that we know are methods. We need
   *  this info because regular functions and static functions have
   *  the same node type (AST_PARAM_LIST) as methods for their param lists,
   *  and we only want to add an artificial "this" node for methods.
   */
  private $method_ids = Array();
  ...
  public function export( $ast, $funcid, $nodeline = 0, $childname = "", $childnum = 0, $namespace = "", $uses = [], $classname = "") : int {
    ...
    if( $ast instanceof ast\Node) {
      ...
      if ( $ast->kind === ast\AST_METHOD) {
		$this->method_ids[$rootnode] = 1;
	  } elseif ( $ast->kind == ast\AST_PARAM_LIST && array_key_exists($funcid, $this->method_ids)) {
		// XXX BRANDONS HACK XXX
		// FYI this node exists even when a method declaration has no parameters
		  $art_param_node = $this->store_node( self::LABEL_AST, "AST_PARAM", "", $nodeline, null, -1, $funcid, $classname, $this->quote_and_escape( $namespace), $nodeendline, "", "");
		  $this->store_rel( $rootnode, $art_param_node, "PARENT_OF");
      	  $node_0 = $this->store_node( self::LABEL_AST, "NULL", null, $nodeline, null, 0, $funcid, $classname, $this->quote_and_escape( $namespace));
		  $this->store_rel( $art_param_node, $node_0, "PARENT_OF");
      	  $name_node = $this->store_node( self::LABEL_AST, "string", null, $nodeline, "this", 1, $funcid, $classname, $this->quote_and_escape( $namespace));
		  $this->store_rel( $art_param_node, $name_node, "PARENT_OF");
      	  $node_2 = $this->store_node( self::LABEL_AST, "NULL", null, $nodeline, null, 2, $funcid, $classname, $this->quote_and_escape( $namespace));
		  $this->store_rel( $art_param_node, $node_2, "PARENT_OF");
	  }
      ...
      foreach( $ast->children as $childrel => $child) {
        ...
        if( $ast->kind === ast\AST_CLASS && $childrel === "stmts") {
          ...
          $this->store_rel( $tnode, $childnode, "PARENT_OF", $childrel); // AST_TOPLEVEL -> AST_STMT_LIST
          $this->store_rel( $rootnode, $tnode, "PARENT_OF", $childrel); // AST_CLASS -> AST_TOPLEVEL
        }
        ...
        elseif( $ast->kind === ast\AST_NAME && $childrel === "name" && $ast->flags !== ast\flags\NAME_FQ) {
          ...
          $this->store_rel( $rootnode, $childnode, "PARENT_OF", $childrel);
        }
