...
class CSVExporter extends Exporter {
  ...
  public function __construct( $format = self::NEO4J_FORMAT, $nodefile = self::NODE_FILE, $relfile = self::REL_FILE, $startcount = 0) {
    ...
    if( $this->format === self::JEXP_FORMAT) {
      ...
      fwrite( $this->rhandle, "start{$this->csv_delim}end{$this->csv_delim}type{$this->csv_delim}child_rel\n");
    }
    else {
      ...
      fwrite( $this->rhandle, ":START_ID{$this->csv_delim}:END_ID{$this->csv_delim}:TYPE{$this->csv_delim}child_rel\n");
    }
  }
   ...
   */
  public function store_rel( $start, $end, $type, $child_rel="") {
